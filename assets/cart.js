...
class CartItems extends HTMLElement {
  ...
  connectedCallback() {
    this.cartUpdateUnsubscriber = subscribe(PUB_SUB_EVENTS.cartUpdate, (event) => {
      ...
      if (
        ...
        event.cartData?.sections?.['cart-drawer']
      ) {
        return;
      }
  ...
  validateQuantity(event) {
    ...
    const isCartInput = Boolean(event.target.closest('cart-items, cart-drawer-items'));
    let message = '';

    if (isCartInput && inputValue === 0) {
      message = '';
        ...
        CartPerformance.measure(`${eventTarget}:paint-updated-sections`, () => {
          ...
          this.getSectionsToRender().forEach((section) => {
            ...
            if (section.id === 'cart-icon-bubble') {
              this.syncBciHeaderCartLinks(sectionInnerHTML);
            }
          });
  ...
  syncBciHeaderCartLinks(sectionInnerHTML) {
    document.querySelectorAll('[data-bci-cart-link]').forEach((cartLink) => {
      cartLink.innerHTML = sectionInnerHTML;
    });
  }
